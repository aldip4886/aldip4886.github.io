...
## Update Revisi Mockup Terakhir

Bagian ini mencatat keputusan desain terbaru agar implementasi backend dan frontend berikutnya konsisten dengan mockup saat ini.

- Struktur peserta tidak lagi memakai tab Overview. Alur peserta dimulai dari `Diagnostik Test`.
- Urutan fase pembelajaran peserta adalah `Diagnostik Test > Materi Pembelajaran > Assignment > Tryout Uji Kompetensi > Feedback`.
- Tab `Assignment dan Rubrik` disederhanakan menjadi `Assignment`; rubrik dan feedback tetap tersedia di dalam konten assignment.
- Sidebar kanan berupa timeline vertikal harus menampilkan progress fase pembelajaran, status terkunci/terbuka/aktif/selesai, nilai aktivitas, dan total progress peserta.
- Sidebar timeline harus dibuat ringkas agar toolbar player, selector slide, tombol download, tombol navigasi, dan kontrol video dapat tampil dalam satu baris pada layar desktop.
- Materi PDF harus ditampilkan per halaman dengan mode default `fit to page`, satu halaman penuh, proporsional, tanpa scrolling PDF internal sebagai perilaku utama.
- Materi video harus mendukung autoplay. Untuk memenuhi kebijakan browser modern, autoplay dijalankan dalam kondisi `muted` dan `playsinline`.
- Toolbar video memakai tombol play/pause, backward/forward informatif dengan ikon double-chevron dan label durasi, seek slider, durasi, serta progress tontonan.
- Seek slider video boleh dikunci sampai peserta menonton video sampai tuntas jika mode sequential enforcement diaktifkan.
- Materi gambar harus memiliki fasilitas zoom in, zoom out, slider level zoom, indikator persentase, dan tombol fit/reset.
- File dari `materi/knowledge` menjadi sumber materi utama, progress modul, FAQ, deep text search, dan knowledge base RAG.
- File dari `materi/resource` menjadi resource tambahan yang juga masuk ke FAQ, deep text search, dan knowledge base RAG.
- HTML mockup utama yang dipakai saat ini adalah `mockup_learning_media_perdagangan_internasional_djbc.html` di root project `C:\Users\ASUS\Documents\Learning media 2`.

### Technology Stack yang Digunakan pada Mockup Saat Ini

Stack berikut adalah stack yang digunakan langsung pada mockup HTML di project directory saat ini.

| Area | Teknologi yang digunakan | Keterangan |
| --- | --- | --- |
| Runtime aplikasi | Static HTML5 single page | Mockup berjalan langsung dari file lokal tanpa build step. |
| Struktur UI | HTML5 semantic layout | Tab, player, sidebar timeline, assignment, quiz, feedback, FAQ, resource, dan chatbot mockup berada dalam satu halaman. |
| Styling | Tailwind CSS via CDN + custom CSS | Tailwind digunakan untuk utility styling; custom CSS dipakai untuk player, PDF fit page, timeline, toolbar, volume popover, dan zoom toolbar. |
| Icon | Font Awesome via CDN | Digunakan untuk icon tab, player, resource, FAQ, chatbot, zoom, download, dan timeline. |
| Interaktivitas | Vanilla JavaScript | Mengatur sequential navigation, quiz, feedback, video control, PDF/image zoom, resource panel, FAQ, chatbot mock, dan assignment grading mock. |
| State persistence | `localStorage` browser | Menyimpan active view, slide terakhir, progress materi, jawaban kuis, skor, assignment draft, dan status submission. |
| PDF viewer | Native browser PDF viewer melalui `iframe` | PDF ditampilkan per halaman dengan parameter `page`, `view=Fit`, `zoom=page-fit`, serta toolbar zoom custom di atas viewer. |
| Video player | HTML5 `<video>` | Autoplay dicoba dengan volume default 30%; fallback mengikuti kebijakan browser bila autoplay bersuara ditolak. |
| Image viewer | Native `<img>` + custom zoom toolbar | Mendukung zoom in/out, slider, indikator persentase, dan fit/reset. |
| Data materi | File lokal `materi/knowledge` dan `materi/resource` | Digunakan untuk manifest mock, resource list, deep search mock, FAQ, dan chatbot/RAG mock. |
| Assignment grading | Rule-based mock grading di JavaScript | Simulasi AI powered grading berdasarkan kata kunci, jumlah kata, dan metadata file upload. |
| Dokumen uji assignment | Markdown file | `mockup_jawaban_assignment_kepabeanan.md` dibuat sebagai file jawaban contoh untuk menguji upload dan grading. |
